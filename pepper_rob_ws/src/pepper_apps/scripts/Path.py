...
class PathToImage:
    def __init__(self):
        self.odom_topic    = rospy.get_param("~odom_topic", "/naoqi_driver/odom")
        self.cmd_topic     = rospy.get_param("~cmd_topic",  "/cmd_vel")
        self.relay_cmd_from= rospy.get_param("~relay_cmd_from", "")
        ...
        self.save_dir     = os.path.expanduser(rospy.get_param("~save_dir", "~/Real Evaluation"))
        ...
        if self.reached_topic:
            rospy.Subscriber(self.reached_topic, Bool, self.reached_cb, queue_size=1)

        if self.relay_cmd_from:
            rospy.loginfo("Relaying Twist from %s -> %s", self.relay_cmd_from, self.cmd_topic)
            rospy.Subscriber(self.relay_cmd_from, Twist, self.relay_cmd_cb, queue_size=1)

        rospy.on_shutdown(self.on_shutdown)
        signal.signal(signal.SIGTERM, self._sig_kill)
        signal.signal(signal.SIGINT,  self._sig_kill)

        rospy.loginfo("PathToImage: odom=%s | cmd_out=%s | save_dir=%s",
                      self.odom_topic, self.cmd_topic, self.save_dir)

    def relay_cmd_cb(self, msg: Twist):
    ...
    def _world_to_img(self, pts_xy):
        ...
        if dx == 0 and dy == 0:
            dx = dy = 1e-2
        xmin -= max(dx, 1e-6) * self.scale_pad
        xmax += max(dx, 1e-6) * self.scale_pad
        ymin -= max(dy, 1e-6) * self.scale_pad
        ymax += max(dy, 1e-6) * self.scale_pad
        dx = xmax - xmin; dy = ymax - ymin
    ...
    def _send_zero_twist(self, times=3, sleep_s=0.05):
        z = Twist()
