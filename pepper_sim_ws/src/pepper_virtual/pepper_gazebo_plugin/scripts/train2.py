...
from gym_pepper_env.gym_env.pepper_env2 import make_improved_env
...
def main():
    ...
    ap.add_argument("--spawn_positions", type=str, default=None,
                    help='This argument is ignored. Spawn position is fixed to (x=-0.02, y=-1.3).')
    ...
    spawn_positions = [{"x": -0.02, "y": -1.3}, {"x": -0.02, "y": 0.7}]
    print(f"**Using fixed spawn position: {spawn_positions}**")
    env_kwargs = dict(
        ...
        yaw_range=(math.radians(100), math.radians(120)),
        debug=(args.n_envs == 1),
    )
