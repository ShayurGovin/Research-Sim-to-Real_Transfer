...
class _RosFrontend:
    ...
    def _img_cb(self, msg: Image):
        ...
        if contours:
            ...
            if areas[max_idx] >= self.min_area:
...
class ImprovedPepperEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 success_distance_thresh=0.8,
                 r_step=-0.005,
                 r_visible=0.05,
                 r_alignment_scale=0.4,
                 alignment_sharpness=15.0,
                 r_approach_scale=0.3,
                 r_exploration=0.01,
                 r_collision=-2.0,
                 r_success=5.0,
                 lower1=(0, 50, 30), upper1=(12, 255, 255),
        ...
        self.r_approach_scale = float(r_approach_scale)
    ...
    def _calculate_reward(self, obs, area_now):
        ...
        if vis >= 0.5:
            ...
            if abs(bearing) < math.radians(30.0):
                approach_bonus = (1.0 - range_norm) * alignment
                reward += self.r_approach_scale * approach_bonus
        else:
        ...
        if self.prev_range is not None:
            range_improvement = range_reading - self.prev_range
            if range_improvement > 0:
                reward += 0.01 * range_improvement

        self.prev_range = range_reading
    ...
    def _check_termination(self, obs):
        ...
        if (vis >= 0.5 and
            abs(bearing) <= self.success_bearing and
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.enable_random_spawn:
            try:
                self._teleport_to_random_fixed()
            except Exception as e:
                rospy.logwarn(f"[Spawn] teleport failed: {e}")

        obs, area = self._get_observation()
        self.prev_range = obs[2] * self.range_clip
        self.prev_area = area

        if obs[2] < 0.4:
            t0 = time.time()
            while (time.time() - t0) < 2.0 and not rospy.is_shutdown():
                self.io.send_cmd(-0.1, 0.3)
                rospy.sleep(0.1)
                obs, area = self._get_observation()
                if obs[2] >= 0.4:
                    break
    ...
    def step(self, action):
        ...
        obs, area = self._get_observation()
        reward = self._calculate_reward(obs, area)
        ...
        if self.debug and (self.step_count % 20 == 0):
            rospy.loginfo(
                "Step=%d v=%.2f w=%.2f | vis=%.1f bear=%.2f range=%.2f conf=%.2f | r=%.3f",
                self.step_count, v, w, obs[0], obs[1], obs[2], obs[3], reward
            )
